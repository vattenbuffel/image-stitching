...
class Undistorter:
    """
    Removes distortion from images. Both barrel and pincushion distortions are handled. 
    To use this undistorter first create it and pass it two images of approximately the same objective but taken from different angles/positions.
    Then call it's undistort function with the image to be undistorted as the argument.
    """
    def __init__(self, img1_rgb, img2_rgb, plot_correspondences = False):
        # Create gray versions of the images
        img1 = cv2.cvtColor(img1_rgb, cv2.COLOR_BGR2GRAY)
        img2 = cv2.cvtColor(img2_rgb, cv2.COLOR_BGR2GRAY)

        sift = cv2.xfeatures2d.SIFT_create()
        # find the keypoints and descriptors with SIFT
        kp1, des1 = sift.detectAndCompute(img1,None) # Why not use the rgb version below? No tutorials do that....
        ...
        # Save xd_hat for undistortion later
        self.xd_hat = xd_hat
        ...
        # Save pixels_to_fill for undistortion later
    ...
    def undistort(self, img):    
        """
        Undistort the rgb image img using the results from the init process. Make sure the input image has the same dimensions as the original images.
        """
        self.undistorted_image[self.pixels_to_fill[1], self.pixels_to_fill[0]] = img[self.xd_hat[1], self.xd_hat[0]].reshape(-1,3)
